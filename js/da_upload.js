let error_code = 0;
let calendar = (date, course_code, course_name, token, time) => {
        ...
        btn.addEventListener("click", async () => {
          ...
          for (let i = 0; i < table.length - 1; i++) {
            ...
            if (error_code != 401 && i == table.length - 2) {
              ...
              alert("Due Dates are successfully Synced to your calender");
              btn.disabled = false;
            }
          }
        });
...
chrome.runtime.onMessage.addListener((request) => {
  if (request.message === "da_upload") {
    try {
      if (da_upload && document.getElementById("semesterSubId")) da_page();
    } catch (error) {
      // console.log(error);
    }
  }
